...
> **این مستند مربوط به کدام برنچ است؟** توسعهٔ فاز ۱ روی برنچ **`step/ph1`** انجام می‌شود، نه روی برنچ پیش‌فرض
> `master`. این مستند `step/ph1` را توصیف می‌کند که بسیار جلوتر از `master` است. بخش
...
**وضعیت فعلی:** فاز ۱ عملاً ساخته شده — آداپترهای پارس‌پک، Use Caseها و ابزارها پیاده‌سازی و در همهٔ پکیج‌ها
یونیت‌تست شده‌اند. اما یک مانع جدی سر راه اجراست: تغییری در بارگذاری `.env` باعث می‌شود سرور بدون وجود فایل
`.env` روی دیسک بالا نیاید — که هم ایمیج داکر را می‌شکند و هم CI را قرمز کرده است. پیش از استقرار حتماً بخش
[وضعیت فعلی پروژه](#sec-14) را بخوانید.
...
> **اول این را بخوانید.** تابع `config.Load()` متد `godotenv.Load()` را صدا می‌زند و اگر فایل `.env` در پوشهٔ
> جاری نباشد پروسه را می‌کشد — یعنی صرفاً export کردن متغیرهای محیطی کافی نیست. به همین دلیل همهٔ مسیرهای زیر
> با ساختن `.env` شروع می‌شوند. این موضوع ایمیج داکر را هم می‌شکند؛ بخش
> [وضعیت فعلی پروژه](#sec-14) را ببینید.

### پیش‌نیازها

- **Go نسخهٔ ۱.۲۶ یا بالاتر** (فایل `go.mod` روی `go 1.26.2` قفل است و CI نسخه را از همان می‌خواند)
...
روی هاست مقصد آن را از داخل پوشه‌ای اجرا کنید که فایل `.env` دارد. دستورهای `make test`، `make vet` و
...
**اما Compose در وضعیت فعلی کار نمی‌کند.** فایل `.env` در `.dockerignore` است، پس ایمیج هیچ `.env`ای ندارد و
همان فراخوانی godotenv بالا کانتینر را در لحظهٔ استارت می‌کُشد — حتی با اینکه Compose متغیرها را از طریق
`env_file` تزریق می‌کند. تا رفع این مشکل، باینری را مستقیم اجرا کنید.
...
توجه کنید که این Skill مجموعه ابزارهای فاز ۱ (حدود ۵۰ ابزار) را مستند می‌کند و برای پوشش قابلیت‌های CDN که
بعداً ذیل ایشیو #24 اضافه شدند به‌روز نشده است — پس چت‌باتی که آن را بارگذاری می‌کند برای هستهٔ VM/DNS/SSL
راهنما دارد، اما برای حدود ۹۰ ابزار لبهٔ CDN هیچ راهنمایی ندارد.
...
| CDN (زون‌ها — **رکوردهای DNS اینجا هستند**) | `https://my.parspack.com/cdnapi` | `WithCDNBaseURL` | `docs/api-specs/parspack-cdn.openapi.yaml` |
| SSL (گردش‌کار سفارش گواهی) | `https://my.parspack.com/sslv2` | `WithSSLBaseURL` | `docs/api-specs/parspack-ssl.openapi.yaml` |
...
برنچ پیش‌فرض مخزن `master` است، اما **کار فاز ۱ روی `step/ph1` توسعه و مرج می‌شود**. این یک پیامد مهم دارد:
بستن خودکار ایشیو با «Closes #N» در گیت‌هاب فقط برای برنچ پیش‌فرض کار می‌کند، بنابراین ورک‌فلوی
`.github/workflows/close-linked-issues.yml` همان اسکنِ کلیدواژه را برای مرج در برنچ‌های غیرپیش‌فرض دوباره
پیاده کرده است. بدون آن، ایشیویی که روی `step/ph1` مرج می‌شود باز و روی `status:in-progress` می‌ماند و بی‌صدا
هر ایشیوی وابسته به آن را قفل می‌کند.
...
کل کار فاز ۱ به شکل ایشیو روی `javadib/do0ps` دنبال می‌شود. هر ایشیو سه دسته برچسب دارد:

- `phase-1` — در محدودهٔ کار است. اگر به‌جایش `backlog` باشد یعنی بعد از فاز ۱
...
| --- | --- | --- |
| `ci.yml` | PR یا push روی `master` یا `step/ph1` | `go build`، `go vet`، `go test ./... -race -cover`؛ golangci-lint v2.12 به‌عنوان job موازی |
| `release.yml` | push روی `master` | go-semantic-release؛ در صورت ریلیز، ورک‌فلوی داکر را صدا می‌زند |
...
| Worker pool: کران‌دار، تخلیهٔ graceful، retry با backoff (حداکثر ۵ تلاش) | پیاده‌سازی و تست شده |
...
| Skill کاربر نهایی (`skills/parspack-infra`) | برای مجموعه ابزارهای فاز ۱ نوشته شده |
| **الزامی بودن `.env`** | **مانع جدی** — پایین‌تر ببینید |
| هندشیک `initialize` در MCP | پیاده نشده |
| متد `JobRepository.ListDue` | پیاده و تست شده اما هیچ‌کس صدایش نمی‌زند؛ چیزی retryهای سررسیدشده را بعد از ری‌استارت poll نمی‌کند |
| LICENSE | با وجود قصد متن‌باز بودن، هنوز کامیت نشده است |

### مانع `.env`

فایل `internal/config/config.go` این کار را می‌کند:
...
متد `godotenv.Load()` وقتی فایل `.env` در پوشهٔ جاری نباشد خطا برمی‌گرداند و `log.Fatal` پروسه را می‌بندد. سه
پیامد دارد:

۱. **`go test ./...` شکست می‌خورد.** پکیج `internal/config` باینری تست را می‌کُشد؛ CI روی `step/ph1` از زمان
   ورود این تغییر قرمز است (کامیت قبلی‌اش سبز بود).

۲. **ایمیج داکر نمی‌تواند بالا بیاید.** فایل `.env` در `.dockerignore` است (به‌درستی — چون رمز دارد)، پس ایمیج
   هرگز چنین فایلی ندارد. تزریق متغیرها با `env_file` در Compose هم کمکی نمی‌کند: خودِ فایل غایب است و کانتینر
   بلافاصله خارج می‌شود. یعنی همان `docker compose up -d --build` که مستند شده کار نمی‌کند.

۳. **صرفاً export کردن متغیرهای محیطی کافی نیست**، روی هیچ هاستی.

راه‌حل این است که نبودِ `.env` عادی تلقی شود — محیط‌های واقعی متغیرها را مستقیم می‌دهند:
...
این کار `.env` را برای توسعهٔ محلی حفظ می‌کند و در عین حال به کانتینر و CI اجازه می‌دهد بدون آن اجرا شوند.

### ناسازگاری‌های کوچک‌تر

- کامنت هدف `run` در `Makefile` هنوز `DO0PS_TOKENS` را می‌گوید؛ نام متغیر `MCP_AUTH_TOKENS` است.
- پکیج `github.com/joho/godotenv` در بلوک `// indirect` فایل `go.mod` قرار دارد، با اینکه `internal/config`
  مستقیماً ایمپورتش می‌کند — `go mod tidy` آن را جابه‌جا می‌کند.
- فایل `AGENTS.md` می‌گوید Go 1.25+ اما `go.mod` روی `go 1.26.2` قفل است و CI نسخه را از `go.mod` می‌خواند.
  `go.mod` را مرجع بدانید.
- Skill کاربر نهایی ابزارهای فاز ۱ را پوشش می‌دهد اما حدود ۹۰ ابزار CDN که ذیل ایشیو #24 اضافه شدند را نه.

---

## <a id="sec-15"></a>۱۵. نقشه راه

محدودهٔ فاز ۱، همان‌طور که در ایشیوتراکر دنبال می‌شود:

| گروه | کار |
| --- | --- |
| A — پایه‌ها | CI و ابزار (#1)، پیکربندی + لاگ ساخت‌یافته (#2)، تایپ‌های دامنه (#3)، portهای هسته (#4) |
| B — آداپترها | انبارهٔ SQLite و recovery (#5)، worker pool صف (#6)، احراز هویت Bearer (#7) |
| C — ترنسپورت | بوت‌استرپ MCP: Fiber v3 + Streamable HTTP + SSE و رجیستری ابزار (#8) |
| D/E — محاسبات پارس‌پک | چرخهٔ عمر VM (#9)، کلید SSH (#10)، فایروال (#11)، Load Balancer (#12)، Reserved IP (#13)، VPC (#14)، اسنپ‌شات (#15) |
| F — CDN و SSL پارس‌پک | بررسی API SSL (#16)، بررسی API CDN (#17)، گردش‌کار سفارش SSL (#18)، زون CDN + رکورد DNS (#19) |
| G — تحویل | ریشهٔ ترکیب و خاموشی graceful (#20)، Dockerfile و compose (#21)، README (#22) |
| H — لایهٔ کاربر نهایی | Skill / سیستم‌پرامپت مجموعهٔ ابزارها (#23) |
| بعد از فاز ۱ | قابلیت‌های CDN فراتر از زون/DNS (#24) — که روی `step/ph1` تحویل شده است |

پس از فاز ۱: آداپترهای ابرآروان و لیارا، و — وقتی دو-سه ارائه‌دهنده وجود داشت و هم‌پوشانی واقعی دیده شد — یک
port مشترک به جای port فعلیِ تک‌ارائه‌دهنده. خودِ `ports.ParspackProvider` با حدود ۱۵۷ متد، دلیل خوبی است که
هنگام آن refactor به تفکیک حوزهٔ قابلیت شکسته شود.

### مواردی که عمداً تصمیم‌گیری نشده‌اند

برای این‌ها پاسخی فرض نکنید؛ از روی انتخاب باز مانده‌اند:

- **مونولیت یا میکروسرویس.** فاز ۱ یک سرویس واحد قابل استقرار است. از قبل آن را تکه‌تکه نکنید و مرزهای ماژول
...
۲. برنچ خود را از **`step/ph1`** بزنید، نه از `master`.

۳. یک ایشیو با برچسب `phase-1` و `status:ready` بردارید و **پیش از نوشتن کد** آن را به `status:in-progress`
   تغییر دهید.
