...
**[English →](README.md)** · **[← معرفی پروژه](../README.md)**

این مستندِ مرجعِ کامل است. برای مرور کوتاه پروژه — اینکه چه می‌کند، چطور نصب می‌شود و هر چیزی کجاست — از
[README ریشه](../README.md) شروع کنید.
...
پریدن به `v1.0.0` فقط minor را بالا می‌برد. این کار `allow-initial-development-versions` است، که
go-semantic-release به‌محض وجود یک *ریلیز* با major بزرگ‌تر یا مساوی ۱ نادیده‌اش می‌گیرد. در این مخزن `v1.0.0`
فقط یک **تگ** است و ریلیزی با آن نام وجود ندارد، و provider به‌جای تگ‌ها ریلیزها را می‌خواند — پس پایه واقعاً
0.0.0 است و نسخهٔ بعدی واقعاً `0.1.0`، همان‌طور که لاگ ورک‌فلو نشان می‌دهد.
